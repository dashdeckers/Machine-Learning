...
def get_experiment(project_name, resume=False):
    ...
    exp['encoder_layers'] = [
        ...
        layers.Flatten(),
        layers.Dense(
            units=exp['input_shape'][1] * 200,
            activation='relu'
        ),
        layers.Dense(
            units=exp['input_shape'][1] * 100,
            activation='relu'
        ),
        layers.Dense(
            units=exp['input_shape'][1] * 50,
            activation='relu'
        ),
        layers.Dense(
            units=exp['input_shape'][1] * 25,
            activation='relu'
        ),
        layers.Dense(
    ...
    exp['decoder_layers'] = [
        ...
        layers.Dense(
            units=int(exp['input_shape'][1] * 25),
            activation='relu'
        ),
        layers.Dense(
            units=int(exp['input_shape'][1] * 50),
            activation='relu'
        ),
        layers.Dense(
            units=int(exp['input_shape'][1] * 100),
            activation='relu'
        ),
        layers.Dense(
            units=int(exp['input_shape'][1] * 200),
            activation='relu'
        ),
        layers.Dense(
            units=17500,
        ...
        layers.Reshape(target_shape=(25, 25, 28)),  # -1 doesn't work
